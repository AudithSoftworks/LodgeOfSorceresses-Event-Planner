    ...
    render = () => {
        ...
        return [
            ...
            <span className="mt-5 col-24 d-flex flex-row flex-nowrap align-items-center justify-content-center" key="buttons" role="group">
                <button type="button" onClick={step === 1 ? () => history.goBack() : () => this.setState({ step: step - 1 })} className="btn btn-primary btn-lg mr-5">
                    Back
                </button>
                <button type="button" onClick={() => this.deleteUserHandler()} className="pl-5 pr-5 ml-5 mr-5 btn btn-danger btn-lg ml-5">
